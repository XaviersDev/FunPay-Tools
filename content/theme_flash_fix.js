...
(async () => {
    ...
    function manageFontImports(settings) {
        if (document.getElementById(FONT_STYLE_ID)) return; // Не создаем, если уже есть
    ...
    function getCustomThemeCss(settings) {
        const bgImageUrl = settings.bgImage ? `url(${settings.bgImage})` : 'none';
        ...
        let baseCss = `
            ...
            html { background-color: #0b0b0b !important; } /* Fallback background */
        ...
        `;
        let themedCss = baseCss.replace(/#ff6d15/gi, settings.bgColor1).replace(/#PRIMARY_COLOR#/gi, settings.bgColor1).replace(/#f4cf78/gi, settings.bgColor2).replace(/#ACCENT_COLOR#/gi, settings.bgColor2).replace(/#f0f0f0/gi, settings.textColor).replace(/#TEXT_COLOR#/gi, settings.textColor).replace(/#2d6bb3/gi, settings.linkColor).replace(/#LINK_COLOR#/gi, settings.linkColor);
        ...
        if (settings.enableCircleCustomization) {
            let circleCss = `.cd-container .cd { transition: transform 0.3s ease, filter 0.3s ease, opacity 0.3s ease; transform: scale(${settings.circleSize / 100}); filter: blur(${settings.circleBlur}px); opacity: ${settings.circleOpacity / 100}; }`;
